  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">Submission activity</span>
      </div>
      ...
      <div className="overflow-x-auto pb-2">
        ...
        <div className="h-4 relative mb-1 ml-8">
          {months.map((m, i) => (
            <div key={i} className="text-[10px] text-gray-400 font-medium absolute" style={{ left: `${m.weekIndex * 13}px` }}>
              {m.label}
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-[2px] relative">
          ...
          <div className="flex flex-col gap-[2px] mr-1 pt-0">
            {['', 'Mon', '', 'Wed', '', 'Fri', ''].map((d, i) => (
              <div key={i} className="h-[11px] text-[9px] text-gray-400 leading-[11px] w-6 text-right pr-1">{d}</div>
            ))}
          </div>
      ...
      {/* Tooltip */}
      {tooltip && (
        <div className="fixed z-50 px-3 py-1.5 rounded-lg bg-gray-900 text-white text-xs shadow-lg pointer-events-none"
...
const InactiveCard = () => (
  <motion.div
    ...
    className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl border border-orange-200/60 p-6"
  >
    <div className="flex items-start gap-4">
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 mt-1">You haven't solved any LeetCode problems today.</p>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <a
          ...
          className="text-sm font-semibold text-gray-900 truncate hover:text-orange-500 transition-colors no-underline block"
        >
          {submission.title}
        </a>
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 hover:text-orange-500 hover:bg-orange-50 transition-colors"
        >
...
const ContestCard = ({ contest }) => (
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <p className="text-sm font-semibold text-gray-900 truncate">{contest.title}</p>
      <div className="flex items-center gap-3 mt-0.5">
        <span className="text-xs text-gray-500">Rank #{contest.ranking}</span>
        <span className="text-xs text-gray-500">{contest.problems_solved}/{contest.total_problems} solved</span>
      </div>
    </div>
    <div className="text-right shrink-0">
      <p className="text-sm font-bold text-gray-900">{Math.round(contest.rating)}</p>
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-5 animate-fade-in-up">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-gray-900">LeetCode Intelligence</h1>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-orange-100 text-orange-600">Live</span>
            </div>
            <p className="text-sm text-gray-500 mt-0.5">Track your coding consistency, submissions, contests, and LeetCode activity.</p>
          </div>
        </div>
      </motion.header>
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <form onSubmit={doAnalyze} className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-3 items-center">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-orange-300 transition placeholder:text-gray-400"
            />
          </div>
          <button
            type="submit"
            disabled={analyzing || !username.trim()}
            className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 text-white text-sm font-semibold shadow-sm hover:shadow-md transition disabled:opacity-50"
          >
        ...
        {error && (
          <div className="mt-3 p-3 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm text-center">{error}</div>
        )}
        ...
        {!profile && !error && !analyzing && (
          <div className="mt-5 flex items-center gap-4 p-5 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50/50">
            ...
            <div>
              <strong className="text-sm text-gray-700 block">No analysis yet</strong>
              <p className="text-xs text-gray-500 mt-0.5">Enter your LeetCode username and hit Analyze to get insights.</p>
            </div>
          </div>
        )}
      </section>
        ...
        {profile && (
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <StatCard icon={Code2} label="Total Solved" value={profile.total_solved} subtitle={`of ${(profile.easy_total + profile.medium_total + profile.hard_total).toLocaleString()}`} color="text-gray-900" />
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-4 rounded-xl bg-orange-50/50 border border-orange-100">
                  ...
                  <p className="text-xs text-gray-500 mt-1 font-medium">Current Streak</p>
                </div>
                <div className="text-center p-4 rounded-xl bg-amber-50/50 border border-amber-100">
                  ...
                  <p className="text-xs text-gray-500 mt-1 font-medium">Max Streak</p>
                </div>
                <div className="text-center p-4 rounded-xl bg-yellow-50/50 border border-yellow-100">
                  <p className="text-3xl font-bold text-yellow-600">{profile.active_days}</p>
                  <p className="text-xs text-gray-500 mt-1 font-medium">Active Days</p>
                </div>
              </div>
            </div>
