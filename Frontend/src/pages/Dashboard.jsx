...
const pCfg = { leetcode: { label: 'LeetCode', icon: Code2, ph: 'Enter LeetCode username' }, codechef: { label: 'CodeChef', icon: Terminal, ph: 'Enter CodeChef username' } };
...
const Dashboard = () => {
  ...
  const [profile, setProfile] = useState({ leetcode_username:'', codechef_username:'', ...(stored||{}) });
  ...
  const uname = plat==='leetcode' ? profile.leetcode_username : profile.codechef_username;
        ...
        <div className="flex flex-col gap-5">
          ...
          <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <form onSubmit={doAnalyze} className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-3 items-center">
              <div className="relative"><PIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={17}/><input type="text" value={uname} placeholder={pCfg[plat].ph} onChange={e=>upd(plat==='leetcode'?'leetcode_username':'codechef_username',e.target.value)} className={inp}/></div>
            ...
            {analysis && analysis.isInactiveToday && analysis.profileHidden && (
              <div className="mt-5 p-5 rounded-xl bg-amber-50 border border-amber-200 flex gap-4 items-start animate-fade-in">
                ...
                <div><span className="text-xs font-bold text-gray-400 uppercase">{pCfg[analysis.platform]?.label}</span><h3 className="font-bold text-gray-900 mt-0.5">{analysis.headline}</h3>{analysis.messages.map((m,i)=><p key={i} className="text-sm text-gray-600 mt-2 leading-relaxed">{m.text}</p>)}</div>
              </div>
            )}
